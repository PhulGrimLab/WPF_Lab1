...
최근 데모/문서 개선을 반영한 현재 판정 포인트입니다.

### 6-1. 선점 해석 정합성

현재 정책:

1. 선점 판정은 RTOS 태스크 간(LOW/HIGH)
2. UI 스레드는 판정 대상 제외

판정:

- 학습용 시뮬레이터로는 충족
- 실제 RTOS 강제 선점 커널 관점에서는 부분/미충족

### 6-2. 관측성 성숙도

아래 항목이 함께 제공되어 학습/디버깅 난도가 낮습니다.

1. 실행 통계 스냅샷
2. trace log
3. 테스트 러너(기능별 시나리오)
4. 선점 데모 인스펙터(TID/활성 태스크)

이 항목은 "학습형 시뮬레이터" 목적에 매우 유리합니다.

### 6-3. 다음 릴리즈 우선순위 제안

1. 단조 증가 시간원(Stopwatch) 기본화
2. 가상 시간 모드(테스트 재현성 향상)
3. 선점 타임라인 리플레이(교육용 가시성 강화)

완료 기준 예시:

1. 동일 시나리오 10회 반복 시 실행 순서 편차가 기준 이하
2. 오버런/양보 시나리오를 타임라인으로 재생 가능
3. 문서-테스트-데모의 용어가 1:1로 대응
